...
(defmacro %create-ta-series (fn series start-idx end-idx output-size &key in-args)
  (with-unique-names (args outputs ta-series start-pos nb-element count ret)
    `(let* ((,args (append (list ,start-idx ,end-idx)
       ...
       (unwind-protect
            (progn
              ...
              (let ((,ret (apply #',fn (append ,args (list ,start-pos ,nb-element) ,outputs))))
                (unless (eq ,ret :success)
                  (error 'talib-error :text (format nil "~a returned ~a" ',fn ,ret)))
                (let ((,count (cffi:mem-ref ,nb-element :int)))
                  (when (> ,count 0)
                    (let ((,ta-series (mapcar (lambda (i)
                                               (mapcar (lambda (out)
         ...
         (when ,start-pos (cffi:foreign-free ,start-pos))
         (when ,nb-element (cffi:foreign-free ,nb-element))))))
