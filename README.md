...
**Fixed**: MAMA now works on all platforms. The `atan` symbol issue was caused by `libta-lib.so` not
finding `libm` (the C math library) at runtime. The bundled `libta-lib.so` in `lib/` already links
against `libm`, so this is resolved automatically. If you are building TA-LIB from source and
encounter this issue, ensure `libm` is linked:

```bash
# When building TA-LIB from source, verify libm is linked:
cd ta-lib && ./configure && make LDFLAGS="-lm" && sudo make install

# Confirm the built library links to libm:
ldd /usr/local/lib/libta-lib.so | grep libm
# Expected output: libm.so.6 => /lib/x86_64-linux-gnu/libm.so.6 (0x...)
```

When passing floating-point parameters (e.g., `fast-limit`, `slow-limit`), use double-float literals
(e.g., `0.05d0`) to avoid precision loss from single-float to double-float coercion.
